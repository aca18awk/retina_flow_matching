...
# Paths
best_model_path = "models/cond_retina/model_best_lunar-shadow-27.pth"
model_name = "lunar-shadow-27"

# Generation Settings
GUIDANCE_SCALES = [0.0, 1.0, 5.0, 10.0, 15.0, 20.0, 30.0, 50.0, 100.0]
BATCH_SIZE = 100
...
with torch.no_grad():
    for guidance_scale in GUIDANCE_SCALES:
        ...
        savedir = os.path.join(
            '/vol/biomedic3/awk24/datasets/Glaucoma_fundus/generated/25_Nov/', 
        ...
        for class_name, total_count in n_samples_dict.items():
            ...
            for batch_idx in tqdm(range(num_batches), desc=f"{class_name}"):
                ...
                generated_class_list = torch.full((current_batch_size,), fill_value=target_label, device=device)
                no_label_list = torch.full_like(generated_class_list, fill_value=3) # Assuming 3 is null class
                ...
                x0_noise = torch.randn(current_batch_size, NO_OF_CHANNELS_IMG, IMG_SIZE, IMG_SIZE, device=device)
